---
eip: 107
title: 通过 HTML 弹窗安全地进行 "eth_sendTransaction" 授权
...
本草案 EIP 描述了一种授权方法的细节，如果由启用 rpc 的以太坊节点提供，将允许常规网站通过 ```eth_sendTransaction```) 发送交易，而无需启用 CORS。相反，用户将被要求通过 HTML 弹窗确认交易。

每个 dapp 想要执行的只读 rpc 调用都被重定向到来自节点域的不可见 iframe，对于 dapp 希望执行的每个交易，都会向用户呈现一个 HTML 弹窗，以允许他/她取消或确认交易。这使得 dapp 能够在不被授予任何特权的情况下连接到节点的 rpc api。这使得用户能够安全地与在日常网页浏览器中运行的 dapp 进行交互，同时他们的账户处于解锁状态。如果账户未解锁，并且节点通过 rpc 允许了 "personal" api，HTML 页面还允许用户输入密码以解锁账户以进行交易。

## 动机

目前，如果用户在使用她/他的日常浏览器访问运行在网站上的 dapp，出于安全原因，dapp 默认将无法访问 rpc api。用户必须为网站的域启用 CORS，以便 dapp 正常工作。不幸的是，如果用户这样做，dapp 将能够从任何解锁的账户发送交易，而无需任何用户同意。换句话说，用户不仅需要更改节点的默认设置，而且还被迫信任 dapp 才能使用它。这当然是不可接受的，并迫使现有的 dapp 依赖于使用变通方法，例如：
- 如果交易是普通的以太转账，用户被要求在一个专用的受信钱包中输入它，比如 "Mist"
- 对于更复杂的情况，用户被要求通过节点命令行界面手动输入交易。

该提案旨在提供一种安全且用户友好的替代方案。

以下是提供的 HTML 弹窗实现的一些截图：

### 账户解锁

当账户已经解锁时，用户会在 dapp 尝试进行的每个交易中看到以下弹窗：

![](../assets/eip-107/authorization.png)

### 账户锁定且未通过 rpc 暴露 "personal" api：

当账户被锁定，并且节点未通过其 rpc 接口提供账户解锁的访问时，将呈现以下弹窗。这并不理想，因为这要求用户知道如何解锁账户：

![](../assets/eip-107/authorization-locked.png)

### 账户锁定但节点通过 rpc 暴露 "personal" api：

更好的选择是询问用户输入他们的密码，但这只有在节点允许通过 rpc 访问 "personal" api 的情况下才可能。在这种情况下，将向用户呈现以下对话框，以便他/她可以通过提供解锁账户所需的密码来接受交易：

![](../assets/eip-107/authorization-password.png)

## 规范

为了使机制正常工作，节点需要通过 http 在 URL \<node url\>/authorization.html 提供一个 HTML 文件。

该文件将被 dapp 以两种不同的模式使用（不可见的 iframe 和弹窗窗口）。

不可见的 iframe 将嵌入在 dapp 中，以允许 dapp 发送其只读 rpc 调用，而无需为 dapp 的网站域启用 CORS。这是通过向 iframe 发送消息（通过 JavaScript ```window.postMessage```) 来实现的，iframe 将执行 rpc 调用。这是可行的，因为 iframe 和节点共享相同的域/端口。

在 iframe 模式下，HTML 文件的 JavaScript 代码将确保不会进行任何需要解锁密钥的调用。这是为了防止 dapp 嵌入不可见的 iframe 并欺骗用户点击确认按钮。
如果 dapp 需要进行 ```eth_sendTransaction``` 调用，dapp 将改为使用相同的 URL 打开一个新窗口。

在这个弹窗窗口模式下，HTML 文件的 JavaScript 代码将允许调用 ```eth_sendTransaction```（但不允许调用 ```eth_sign```, 因为没有安全地向用户显示要签名的交易的有意义内容的方法）。但在将调用直接发送到节点之前，将呈现一个确认对话框，显示发送者和接收者地址，以及转账金额和潜在的 gas 费用。在用户批准后，请求将被发送，结果将返回给 dapp。如果用户取消请求，将返回错误。

HTML 页面还检查 "personal" api 的可用性，如果可用，将在必要时要求用户解锁账户。解锁是临时的（3 秒），因此如果在此短时间结束之前尝试进行交易，将再次要求输入密码。

在 iframe 模式和窗口模式下，与 dapp 的通信是通过 ```window.postMessage```.
iframe/窗口发送的第一条消息是包含字符串 "ready" 的消息，以让 dapp 知道它现在接受消息。然后，dapp 可以通过发送消息使用以下对象开始执行 rpc 调用：
```
{
...
对于 ```eth_sendTransaction```，需要在 rpc 参数中设置 "gas"、"gasPrice" 和 "from" 字段，以便窗口可以显示正确的值。如果没有传递所有这些字段，窗口将返回错误。

在接收到此类消息后，iframe 将执行实际的 rpc 调用到节点，但仅当该调用是只读调用（不需要解锁密钥）时。如果不是，它将返回错误。另一方面，窗口将仅接受 ```eth_sendTransaction``` 调用，但将显示一个对话框，以便用户可以接受或取消请求。

在所有情况下，iframe/窗口将使用以下对象向 dapp 发送消息：
```
{
...
错误对象不能是 JavaScript 错误对象，因为 postMessage 的限制。相反，它是
```
{
...
该提案的设计因其简单性和安全性而被选择。之前的一个想法是使用类似 oauth 的协议，以便用户接受或拒绝交易请求。这将需要对节点进行更深层次的代码更改，一些 geth 贡献者认为这样的更改不适合 geth 代码库，因为这将需要 dapp 感知的代码。
当前设计则具有非常简单的实现（自包含的 HTML 文件，可以在节点的实现之间共享），其安全性由浏览器的跨域政策保证。

使用 iframe/窗口是为了兼顾安全性和用户友好性。不可见的 iframe 允许 dapp 执行只读调用，而无需用户输入，而窗口则确保在进行调用之前获得用户的批准。虽然我们可以通过使 iframe 确认使用原生浏览器的 ```window.confirm``` 对话框来实现，但这将阻止使用当前设计允许的更优雅的确认弹窗。此外，```window.confirm``` 在某些浏览器中并不安全，因为它将焦点放在接受选项上，并且可以自动触发（https://bugs.chromium.org/p/chromium/issues/detail?id=260653）。
## 实现

为了实现这个设计，需要在 URL \<node url\>/authorization.html 提供以下 HTML 文件或等效文件

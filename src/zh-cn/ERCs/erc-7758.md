...
一组函数，用于通过符合 [EIP-712](./eip-712.md) 类型消息签名规范的签名，启用元交易和与 [ERC-20](./eip-20.md) 代币合约的原子交互。
...
新合约可以直接利用此提案来创建原子交易，但现有合约仍可能依赖于传统的 [ERC-20](./eip-20.md) 授权模式（`approve`/`transferFrom`）。

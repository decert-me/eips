...
它向 Vault 添加了一个新的 `share` 方法，以允许 [ERC-20](./eip-20.md) 依赖项外部化。
...
一个未被 [ERC-4626](./eip-4626.md) 支持的用例是具有多个资产或入口点的 Vaults，例如流动性提供者 (LP) 代币。这些通常由于 ERC-4626 要求自身为 [ERC-20](./eip-20.md) 而显得笨重或不合规。
...
所有 [ERC-7575](./eip-7575.md) Vaults 必须实现 [ERC-4626](./eip-4626.md)，不包括 [ERC-20](./eip-20.md) 方法和事件。

#### share

在存入 Vault 时接收到的基础 `share` 的地址。必须返回 Vault 的 [ERC-20](./eip-20.md) 份额表示的地址。
...
多资产 Vaults 必须在每个入口点实现 `share` 方法。入口点不应是 [ERC-20](./eip-20.md)。

### Pipes

Pipes 在 Vault 外部的单一 `asset` 和 `share` 之间进行转换，这两者都是 [ERC-20](./eip-20.md) 代币。
...
[ERC-20](./eip-20.md) 实现的 `share` 应实现一个 `vault` 方法，返回特定 `asset` 的 Vault 地址。
...
### 外部化 [ERC-20](./eip-20.md) 依赖的能力

通过允许 `share != address(this)`，Vault 可以有一个外部合约来管理份额的 [ERC-20](./eip-20.md) 功能。在多资产的情况下，这避免了如果每个 Vault 本身都被要求是 [ERC-20](./eip-20.md) 可能引起的混淆，这可能会让集成者和前端感到困惑。

这种方法还使得创建新类型的 Vaults 成为可能，例如 Pipes，促进两个外部 [ERC-20](./eip-20.md) 代币之间的转换。这些 Pipes 可以是单向的（即仅用于通过存入/铸造将资产转换为份额，或通过赎回/提取将份额转换为资产）或双向的，适用于入口和出口流。
...
[ERC-7575](./eip-7575.md) Vaults 与 [ERC-4626](./eip-4626.md) 不完全兼容，因为 [ERC-20](./eip-20.md) 功能已被移除。
...
不合规的 [ERC-20](./eip-20.md) Vaults 必须小心支持 `owner` 不是 `msg.sender` 的赎回流程，因为如果 Vault 和份额是分开的合约， [ERC-20](./eip-20.md) 授权流程本身无法正常工作。可以通过将 Vault 设置为份额代币的受信转发器来实现，使用 [ERC-2771](./eip-2771.md)。
